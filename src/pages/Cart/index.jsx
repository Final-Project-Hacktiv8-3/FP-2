...
export const Cart = () => {
    ...
    const handlePayments = ()=>{
        ...
        carts.cartItems.forEach((cartItem) => {
            const matchedProduct = latest.latest.find((latestData) => latestData.id === cartItem.id);
        
            if (matchedProduct && cartItem.quantity <= matchedProduct.quantity) {
              dispatch(addToRekapan(cartItem.id, cartItem.userId, cartItem.quantity, cartItem.image, cartItem.price, cartItem.title));
    ...
    const total = carts.cartItems.reduce((accumulator, cart) => {
        const matchedStuff = tempProducts.find(stuff => cart.id === stuff.id && cart.userId === userId);
        if (matchedStuff) {
    ...
    return (
        ...
        <div className="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 dark:bg-gray-800 dark:border-gray-700 mt-20 ml-12">
