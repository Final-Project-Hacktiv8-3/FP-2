...
import { addToRekapan } from "../../redux/rekapan/action";
import { subtractProducts } from "../../redux/products/action";
import { deleteCart } from "../../redux/adding/action";
import { latestUpdate } from "../../redux/latest/action";
  ...
  const handlePayments = () => {
    carts.cartItems.forEach((cartItem) => {
      const matchedProduct = latest.latest.find(
        (latestData) => latestData.id === cartItem.id,
      );

      if (matchedProduct && cartItem.quantity <= matchedProduct.quantity) {
        dispatch(
          addToRekapan(
            cartItem.id,
            cartItem.userId,
        ...
        dispatch(subtractProducts(cartItem.id, cartItem.quantity));
        dispatch(latestUpdate(cartItem.id, cartItem.quantity));
        dispatch(deleteCart(cartItem.id));
        setIsShowModal(!isShowModal);
      } else {
        // setShow(true);
        // setShowStuff(cartItem.title)
      }
    });
  };

  const total = carts.cartItems.reduce((accumulator, cart) => {
    const matchedStuff = tempProducts.find(
      (stuff) => cart.id === stuff.id && cart.userId === userId,
    );
    if (matchedStuff) {
  ...
  return (
    <>
      ...
      <div className="flex">
          ...
          <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {carts.cartItems.length > 0 ? (
                ...
                <tr className="border-b bg-white dark:border-gray-700 dark:bg-gray-900">
                  <h1 className="ml-52">Barang tidak ditemukan</h1>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="ml-12 mt-20 w-full max-w-sm rounded-lg border border-gray-200 bg-white p-4 shadow dark:border-gray-700 dark:bg-gray-800 sm:p-6">
          ...
          <button
            type="button"
            className="-ml-1 rounded-lg bg-color px-36 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4   focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
